...
import { ArrowUpRight, Globe, Cpu, Users, Terminal, Check, Activity, BarChart, Shield, Zap, Sparkles, RefreshCw } from "lucide-react";
...
const Extechnology = () => {
  ...
  const [cartStatus, setCartStatus] = useState<"idle" | "processing" | "success">("idle");
  ...
  const [logs, setLogs] = useState<LogLine[]>([
    { time: "17:14:02", type: "INFO", message: "Kernel initialized successfully." },
    { time: "17:14:03", type: "SUCCESS", message: "Connection established with primary DB node." },
    { time: "17:14:05", type: "INFO", message: "Listening for requests on port 8080..." }
  ]);
  ...
  const handleDispatch = () => {
    ...
    const newLogs: LogLine[] = [
      ...logs,
      { time, type: "INFO", message: `Initiating agent dispatcher #${apiCount + 1}...` },
      { time, type: "SUCCESS", message: `Agent #${apiCount + 1} completed execution. Speed: 12ms.` }
    ];
  ...
  const moveDeal = (id: number) => {
    setCrmDeads(prev =>
      prev.map(d => (d.id === id ? { ...d, stage: d.stage === "pipeline" ? "won" : "pipeline" } : d))
    );
  };
  ...
  const [marketingMetric, setMarketingMetric] = useState<"traffic" | "conversions" | "roas">("traffic");
  ...
  const getMarketingBars = () => {
    switch (marketingMetric) {
      ...
      case "conversions":
        return [
          { name: "Q1", value: "1.2%", height: "h-[20%]", color: "bg-white/40" },
          { name: "Q2", value: "2.8%", height: "h-[45%]", color: "bg-white/60" },
          { name: "Q3", value: "6.2%", height: "h-[75%]", color: "bg-white/80" },
          { name: "Q4", value: "9.5%", height: "h-[95%]", color: "bg-accent" },
        ];
      case "roas":
        return [
          { name: "Q1", value: "1.5x", height: "h-[25%]", color: "bg-white/40" },
          { name: "Q2", value: "2.4x", height: "h-[48%]", color: "bg-white/60" },
          { name: "Q3", value: "3.8x", height: "h-[76%]", color: "bg-white/80" },
  ...
  // Tech Explorer State
  const [activeCategory, setActiveCategory] = useState<"websites" | "applications" | "crms" | "marketing">("websites");

  const techStacks = {
    websites: [
      { name: "Next.js / React 19", role: "Core SSR / Server Components" },
      { name: "Tailwind CSS v4", role: "Next-gen CSS utility styling" },
      { name: "Shopify Storefront", role: "Headless e-commerce commerce pipeline" },
      { name: "Stripe SDK", role: "Secure globally-distributed checkouts" },
    ],
    applications: [
      { name: "Node.js / Express / Bun", role: "Ultra-fast response runtime APIs" },
      { name: "Golang / Python", role: "Distributed workers & compute systems" },
      { name: "PostgreSQL & Redis", role: "Persistent structures & cache layers" },
      { name: "Docker Engine", role: "Strict environments & scale controllers" },
    ],
    crms: [
      { name: "Salesforce Partner API", role: "Enterprise sync integrations" },
      { name: "HubSpot SDK Connector", role: "Customer lifecycle data pipelining" },
      { name: "PgVector Embeddings", role: "RAG & LLM-driven query databases" },
      { name: "Custom Webhook Hubs", role: "Immediate, latency-free CRM triggers" },
    ],
    marketing: [
      { name: "Segment.io Platform", role: "Central behavioral data engine" },
      { name: "Custom Mailing Journeys", role: "Automated newsletter & drip engine" },
      { name: "Google Analytics 4 API", role: "Structured event tracking metrics" },
    ...
    <div ref={revealRef} className="min-h-screen bg-background text-foreground font-sans relative overflow-hidden">
      ...
      <section id="top" className="relative min-h-screen flex flex-col justify-between pt-32 pb-16 px-5 sm:px-8 max-w-7xl mx-auto z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-auto mb-auto pt-6">
          ...
          <div data-reveal className="lg:col-span-7 flex flex-col justify-center space-y-8">
            ...
            <div className="space-y-4">
              <p className="text-accent text-sm sm:text-base font-mono uppercase tracking-[0.3em] font-semibold">
              ...
              <h1 className="text-balance font-display text-5xl sm:text-7xl lg:text-8xl font-normal leading-[0.9] tracking-tight uppercase">
                Your Website, <br className="hidden sm:inline" />
            ...
            <p className="max-w-xl text-lg sm:text-xl leading-relaxed text-muted-foreground">
              Create intelligent digital experiences that boost productivity, conversions, and business performance. We build clean, high-performance engines tailored to power modern digital enterprises.
            </p>
          ...
          <div data-reveal className="lg:col-span-5 flex items-center">
            <div className="w-full border border-white/10 bg-card/60 backdrop-blur-md p-6 space-y-6 rounded-none relative">
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <span className="font-mono text-xs text-white/50 tracking-wider">CONSOLE // EXTECH_SYS</span>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 bg-emerald-500 animate-pulse" />
                  <span className="font-mono text-[10px] text-emerald-500">SYS_ONLINE</span>
                </div>
              </div>
              ...
              <div className="space-y-4 font-mono text-xs">
                ...
                <div className="flex justify-between border-b border-white/5 pb-2">
                  <span className="text-muted-foreground">SERVICES INITIALIZED</span>
                ...
                <div className="flex justify-between border-b border-white/5 pb-2">
                  <span className="text-muted-foreground">TARGET DEPLOYMENTS</span>
              ...
              <div className="bg-white/5 p-3 border border-white/10 font-mono text-[10px] text-muted-foreground leading-relaxed">
                <span className="text-white font-bold">INFO:</span> Systems routing optimized. Sharp grid layouts enabled. Circular layouts suppressed per protocol: <span className="text-accent font-semibold">AVOID_ROUNDED_EDGES = TRUE</span>.
              </div>
            </div>
          </div>
        </div>
        ...
        <div data-reveal className="grid grid-cols-2 md:grid-cols-4 border border-white/10 bg-white/2 divide-x divide-y md:divide-y-0 divide-white/10 mt-16 font-mono text-xs uppercase tracking-wider">
          <a href="#websites" className="p-5 flex items-center justify-between hover:bg-white/5 transition-colors group">
            <span>01 // Websites</span>
            <ArrowUpRight className="w-4 h-4 text-muted-foreground group-hover:text-white transition-colors" />
          </a>
          <a href="#applications" className="p-5 flex items-center justify-between hover:bg-white/5 transition-colors group">
            <span>02 // Applications</span>
            <ArrowUpRight className="w-4 h-4 text-muted-foreground group-hover:text-white transition-colors" />
          </a>
          <a href="#crms" className="p-5 flex items-center justify-between hover:bg-white/5 transition-colors group">
            <span>03 // CRMs</span>
            <ArrowUpRight className="w-4 h-4 text-muted-foreground group-hover:text-white transition-colors" />
          </a>
          <a href="#marketing" className="p-5 flex items-center justify-between hover:bg-white/5 transition-colors group">
      ...
      {/* 01. Websites */}
      <section id="websites" className="relative border-t border-white/10 scroll-mt-20 py-24 sm:py-32">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            ...
            <div data-reveal className="lg:col-span-6 space-y-6">
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs uppercase text-accent">[ 01 // CORE PRODUCT ]</span>
              ...
              <p className="text-muted-foreground text-base sm:text-lg leading-relaxed">
                Sell your products to customers anywhere, anytime, without location limits. Turn visitors into customers with a fast, secure, and convenient online shopping experience.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  ...
                  <div className="text-muted-foreground">Deploy shops cross-region.</div>
                </div>
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  <div className="text-white font-semibold">CONVERSION SPEED</div>
                  <div className="text-muted-foreground">Sub-second page speeds.</div>
                </div>
              </div>
            </div>
              ...
              <div className="border border-white/10 bg-card p-6 space-y-6 relative rounded-none">
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs uppercase text-white font-bold">STOREFRONT.SIMULATOR</span>
                  </div>
                ...
                <div className="border border-white/10 p-4 bg-background space-y-4 rounded-none">
                  <div className="aspect-video w-full bg-white/5 flex items-center justify-center relative rounded-none border border-white/5">
                    ...
                    <span className="font-mono text-xs text-white/30 uppercase tracking-widest">[ ex_commerce_package ]</span>
                  </div>
                  
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-display font-medium text-base text-white">ExCommerce Premium Suite</h4>
                      <p className="text-xs text-muted-foreground font-mono mt-0.5">Automated web sales license</p>
                    </div>
                    <span className="font-mono text-sm text-accent font-bold">$12,999.00</span>
                  </div>
                    ...
                    {cartStatus === "success" && (
                      <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 space-y-2 rounded-none">
                        ...
                        <p className="text-[10px] text-emerald-400/80 font-mono leading-relaxed">
                          Secure token generated. Customer redirects fired. E-commerce conversion registered.
                        </p>
                        <button
      ...
      {/* 02. Applications */}
      <section id="applications" className="relative border-t border-white/10 scroll-mt-20 py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
              ...
              <div className="border border-white/10 bg-card p-6 space-y-6 relative rounded-none">
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs uppercase text-white font-bold">APP.PROCESS_MONITOR</span>
                  </div>
                  <button 
                ...
                <div className="bg-background border border-white/10 p-4 font-mono text-[11px] space-y-2 min-h-[180px] flex flex-col justify-end rounded-none">
                  {logs.map((log, idx) => (
                    <div key={idx} className="flex gap-2 leading-relaxed">
                      ...
                      <span className={log.type === "SUCCESS" ? "text-emerald-400" : "text-muted-foreground"}>
                        {log.type}:
                      </span>
            ...
            <div data-reveal className="lg:col-span-6 lg:order-1 space-y-6">
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs uppercase text-accent">[ 02 // AGENT WORKFLOWS ]</span>
                <span className="h-px flex-grow bg-white/10" />
              </div>
              ...
              <p className="text-muted-foreground text-base sm:text-lg leading-relaxed">
                Build stronger customer connections with intelligent applications designed for modern businesses. We develop cloud-native, real-time products crafted to operate smoothly at scale.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  <div className="text-white font-semibold">SECURE SOCKETS</div>
                  <div className="text-muted-foreground">Instantly synchronized connections.</div>
                </div>
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  <div className="text-white font-semibold">ROBUST CORE</div>
                  <div className="text-muted-foreground">Built to execute 24/7 without fail.</div>
                </div>
              </div>
            </div>
      ...
      {/* 03. CRMs */}
      <section id="crms" className="relative border-t border-white/10 scroll-mt-20 py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            ...
            <div data-reveal className="lg:col-span-6 space-y-6">
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs uppercase text-accent">[ 03 // DATA PIPELINE ]</span>
                <span className="h-px flex-grow bg-white/10" />
              </div>
              ...
              <p className="text-muted-foreground text-base sm:text-lg leading-relaxed">
                Build stronger relationships and close more deals with the power of CRM. Synchronize lead acquisition pipelines, automate follow-ups, and structure customer profiles into a robust central directory.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  <div className="text-white font-semibold">SYNC ENGINE</div>
                  <div className="text-muted-foreground">Bi-directional automated hooks.</div>
                </div>
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  <div className="text-white font-semibold">LEAD ATTRIBUTION</div>
                  <div className="text-muted-foreground">Track absolute ROI of source.</div>
                </div>
              </div>
            </div>
              ...
              <div className="border border-white/10 bg-card p-6 space-y-6 relative rounded-none">
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs uppercase text-white font-bold">PIPELINE.CONTROLLER</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="border border-white/10 bg-background p-3 space-y-3 rounded-none">
                    <div className="border-b border-white/10 pb-2 flex justify-between items-center">
                      <span className="font-mono text-[10px] text-muted-foreground uppercase">LEADS_FLOW</span>
                    ...
                    {crmDeads.filter(d => d.stage === "pipeline").map(deal => (
                      <div 
                        key={deal.id} 
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center text-[10px] font-mono text-muted-foreground">
                          ...
                          <span className="text-accent font-semibold group-hover:underline">[WIN]</span>
                        </div>
                      </div>
                    ))}
                    {crmDeads.filter(d => d.stage === "pipeline").length === 0 && (
                  ...
                  <div className="border border-white/10 bg-background p-3 space-y-3 rounded-none">
                    <div className="border-b border-white/10 pb-2 flex justify-between items-center">
                      <span className="font-mono text-[10px] text-muted-foreground uppercase">CLOSED_WON</span>
                    ...
                    {crmDeads.filter(d => d.stage === "won").map(deal => (
                      <div 
                        key={deal.id}
                        onClick={() => moveDeal(deal.id)}
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center text-[10px] font-mono text-emerald-400/80">
                          <span>VALUE: ${deal.value}</span>
                          <span className="text-white/50 underline">[RESET]</span>
                        </div>
                      </div>
                    ))}
                ...
                <div className="text-[10px] text-muted-foreground font-mono leading-relaxed bg-white/5 p-2.5 border border-white/10">
                  <span className="text-accent font-bold">ENGAGEMENT TIP:</span> Click on a lead card to update its pipeline status instantly. Watch total won revenue scale up.
                </div>
              </div>
            </div>
      ...
      <section id="marketing" className="relative border-t border-white/10 scroll-mt-20 py-24 sm:py-32">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
              ...
              <div className="border border-white/10 bg-card p-6 space-y-6 relative rounded-none">
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs uppercase text-white font-bold">METRICS.DISPATCHER</span>
                  </div>
                ...
                <div className="bg-background border border-white/10 p-6 h-[200px] flex items-end justify-between relative rounded-none">
                  ...
                  {getMarketingBars().map((bar, idx) => (
                    <div key={idx} className="flex flex-col items-center w-[18%] h-full justify-end z-10 group relative">
                      {/* Floating tooltip */}
                      ...
                      <div className={`w-full ${bar.color} ${bar.height} transition-all duration-500 ease-out rounded-none`} />
                      <span className="font-mono text-[10px] text-muted-foreground mt-2 uppercase">{bar.name}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-[10px] text-muted-foreground font-mono flex items-center justify-between">
                  ...
                  <span className="text-accent animate-pulse font-bold">STATUS: TRACKING_OK</span>
                </div>
              </div>
            </div>
            ...
            <div data-reveal className="lg:col-span-6 lg:order-1 space-y-6">
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs uppercase text-accent">[ 04 // GROWTH SYSTEMS ]</span>
                <span className="h-px flex-grow bg-white/10" />
              </div>
              ...
              <p className="text-muted-foreground text-base sm:text-lg leading-relaxed">
                Scalable enterprise software tailored to your unique business processes, increasing efficiency and productivity. Run automated user-journey workflows, custom attribution scripts, and high-volume email funnels.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  <div className="text-white font-semibold">DATA VISIBILITY</div>
                  <div className="text-muted-foreground">Unified customer journey logs.</div>
                </div>
                <div className="border border-white/5 bg-white/2 p-4 font-mono text-xs space-y-1">
                  <div className="text-white font-semibold">SCALE SYSTEM</div>
                  <div className="text-muted-foreground">Orchestrate custom pipelines.</div>
                </div>
              </div>
            </div>
      ...
      <section id="tech-explorer" className="relative border-t border-white/10 py-24 sm:py-32 scroll-mt-20">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          
          <div data-reveal className="max-w-2xl mb-16 space-y-4">
            <span className="font-mono text-xs uppercase text-accent">[ SYSTEM MATRIX & STACKS ]</span>
            <h2 className="font-display text-3xl sm:text-5xl uppercase tracking-tight">
              Unified Technology Stack
            </h2>
            <p className="text-muted-foreground text-base sm:text-lg">
              Explore the battle-tested technologies and frameworks we deploy across our four core capabilities.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 border border-white/10 rounded-none bg-white/2 overflow-hidden">
            ...
            <div className="lg:col-span-4 border-r border-white/10 flex flex-col divide-y divide-white/10 font-mono text-xs uppercase tracking-wider bg-black/40">
              ...
              >
                <span>01 // Websites</span>
                <Globe className={`w-4 h-4 ${activeCategory === "websites" ? "text-background" : "text-muted-foreground"}`} />
              </button>
              <button
              ...
              >
                <span>02 // Applications</span>
                <Cpu className={`w-4 h-4 ${activeCategory === "applications" ? "text-background" : "text-muted-foreground"}`} />
              </button>
              <button
              ...
              >
                <span>03 // CRMs</span>
                <Users className={`w-4 h-4 ${activeCategory === "crms" ? "text-background" : "text-muted-foreground"}`} />
              </button>
              <button
              ...
              >
                ...
                <Terminal className={`w-4 h-4 ${activeCategory === "marketing" ? "text-background" : "text-muted-foreground"}`} />
              </button>
            </div>
            ...
            <div className="lg:col-span-8 p-6 sm:p-8 flex flex-col justify-between min-h-[300px]">
              <div>
                <div className="flex justify-between items-center border-b border-white/10 pb-4 mb-6">
                  <span className="font-mono text-xs uppercase tracking-widest text-muted-foreground">ACTIVE_STACK // {activeCategory.toUpperCase()}</span>
                  <span className="text-[10px] font-mono text-accent">[ SYSTEM LEVEL ]</span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {techStacks[activeCategory].map((tech, idx) => (
                    <div key={idx} className="border border-white/5 bg-white/2 p-4 space-y-2 rounded-none hover:border-white/20 transition-all duration-300">
      ...
      <section className="relative border-t border-white/10 py-24 sm:py-32 bg-white/[0.01]">
        <div className="max-w-4xl mx-auto px-5 text-center space-y-8">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto text-base sm:text-lg">
            Connect with our team to scope your application framework, integration pipelines, and custom dashboards.
          </p>
